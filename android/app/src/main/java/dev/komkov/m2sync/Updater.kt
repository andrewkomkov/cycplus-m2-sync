...
data class UpdateProgress(val stage: Stage, val bytes: Long = 0, val total: Long = 0) {
...
object Updater {
    ...
    fun start(ctx: Context, update: UpdateChecker.Update) {
    ...
    private suspend fun run(ctx: Context, update: UpdateChecker.Update, apkUrl: String) {
        try {
     ...
     */
    private suspend fun verify(apk: File, sha256Url: String?): Boolean = withContext(Dispatchers.IO) {
        if (sha256Url == null) return@withContext true
        val published = runCatching {
            val connection = (URL(sha256Url).openConnection() as HttpURLConnection).apply {
