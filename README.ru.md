...
сразу туда — заезды видит Google Health и всё остальное, что читает это хранилище, а файлы
остаются у тебя.

Отдельно про Strava: её связка с Health Connect односторонняя. Strava пишет туда активности и
читает оттуда вес, но активности из него не забирает — этим путём заезды к ней не попадут. Залить
их можно только вручную, файлом: кнопка «Поделиться» его отдаёт. Своей выгрузки в приложении нет
потому, что с июня 2026 API Strava на уровне Standard, куда попадает такое приложение, требует
платной подписки Strava.
...
Версия для iOS лежит в [`ios/`](ios). Она находит велокомпьютер, качает новые заезды по Bluetooth
и сохраняет их в «Здоровье» как велотренировки на улице: маршрут, пульс, каденс, скорость,
дистанция и набор высоты, а остановки размечены паузами, так что время в движении совпадает с
`.fit` до секунды. Тренировки видны и в «Фитнесе», повторный синк дублей не создаёт. Калории,
экспорт `.fit`, карта и графики появятся следующими изменениями. Нужен iOS 17 и новее, собирается
из исходников:
