...
которых M2 не пишет, считаются по пульсу, а где его нет — по скорости, и сохраняются как активная
энергия; вес, год рождения и пол берутся из «Здоровья» или из профиля в приложении. По нажатию
на заезд открываются маршрут на карте Apple, итоги и графики высоты, скорости, пульса и каденса,
которые можно протягивать пальцем. Исходный `.fit` отправляется через «Поделиться» — один заезд или
несколько выбранных, с именами вида `2026-07-24_10-30_40.99km_cycplus-m2.fit`. После первого синка по кнопке
приложение само синхронизируется при открытии, выключается это в меню «Ещё». Нужен iOS 17 и новее,
